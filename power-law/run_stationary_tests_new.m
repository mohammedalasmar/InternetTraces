...
for ii = 19:19
    ...
    %      time_scales_T = [0.005, 0.05, 0.1 , 0.15, 0.2, 0.25, 0.3]; %% for caida onlyyyy
...
% P = polyfit(t,dataRate/10^6,1);
%     yfit = P(1)*t+P(2);
% hold on;
%     plot(t,yfit,'r-.', 'LineWidth', 5);
        ...
        h_kpss_all(t_i)=h_kpss ; pValue_kpss_all(t_i)=pValue_kpss; stat_kpss_all(t_i)=stat_kpss;cValue_kpss_all(t_i)=cValue_kpss;
        ...
        h_adf_all(t_i) = h_adf; pValue_adf_all(t_i)= pValue_adf;  stat_adf_all(t_i)=stat_adf;cValue_adf_all(t_i)=cValue_adf;
        h_pp_all(t_i) = h_pp; pValue_pp_all(t_i)= pValue_pp;  stat_pp_all(t_i)=stat_pp;cValue_pp_all(t_i)=cValue_pp;
