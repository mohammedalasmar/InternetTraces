...
switch f_dattype,
    ...
    case 'REAL',
        ...
        %figure;
        h(1) = loglog(c(:,1),c(:,2),'-b','LineWidth',3, 'MarkerSize',8,'MarkerFaceColor',[1 1 1]); hold on;
        ...
        xlabel('x (bps)','FontSize',16)
        % MOH added
       % legend('Data Rate','power-law fit')
    ...
    case 'INTS',
       ...
       % figure;
        h(1) = loglog(c(:,1),c(:,2),'-','MarkerSize',8,'MarkerFaceColor',[1 1 1]); hold on;
        %h(2) = loglog(cf(:,1),cf(:,2),'k--','LineWidth',2); hold off;
        xr  = [10.^floor(log10(min(x))) 10.^ceil(log10(max(x)))];
        xrt = (round(log10(xr(1))):2:round(log10(xr(2))));
        if length(xrt)<4, xrt = (round(log10(xr(1))):1:round(log10(xr(2)))); end;
