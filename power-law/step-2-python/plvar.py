...
def plvar(x,*varargin):
    ...
    while i<len(varargin): 
      ...
      #      print ("Ignoring invalid argument") 
        i = i+1 
    ...
    if f_dattype== 'REAL':
        ...
        for B in range(0,Bt):
            ...
            if not quiet:
                print ('['+str(B+1)+']\tntail = ',round(mean(bofA),3),' (',round(std(bofA),3),')','\txmin = ', round(mean(bofB),3),' (',round(std(bofB),3),')','\talpha = ',round(mean(bofC),3),' (',round(std(bofC),3),')')
